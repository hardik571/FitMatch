...
const ChatAssistant: React.FC<ChatAssistantProps> = ({ activeImage, language = 'en' }) => {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      ...
      role: 'model',
      text: language === 'hi'
        ? 'नमस्ते! मैं आपका पर्सनल AI स्टाइलिस्ट हूँ। मुझसे कपड़ों, रंगों या स्टाइल के बारे में कुछ भी पूछें!'
        : 'Hi! I am your personal AI Stylist. Ask me for outfit ideas, color combinations, or send me a photo description!',
      timestamp: Date.now()
    }
  ]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const response = await chatWithStylist(history, userMsg.text, currentImage || undefined, language as Language);

      const botMsg: ChatMessage = {
        id: (Date.now() + 1).toString(),
        role: 'model',
        text: response || (language === 'hi' ? "माफ़ कीजिये, मैं समझ नहीं पाया।" : "I couldn't quite catch that. Could you rephrase?"),
        timestamp: Date.now()
      };
  ...
  const uiText = {
    placeholder: language === 'hi' ? 'स्टाइल के बारे में पूछें...' : 'Ask for style advice...',
    imageAttached: language === 'hi' ? 'फोटो संलग्न' : 'Image attached',
    analyzing: language === 'hi' ? 'AI देख रहा है' : 'AI will analyze this',
    chatBtn: language === 'hi' ? 'AI चैट' : 'Chat with AI'
  };
  ...
  if (!isOpen) {
    return (
      <button
        ...
        className="fixed bottom-24 right-4 md:bottom-6 md:right-6 bg-gradient-to-r from-indigo-600 to-purple-600 text-white p-4 rounded-full shadow-xl hover:scale-105 transition z-50 flex items-center gap-2"
      >
        <MessageCircle size={28} />
        <span className="hidden md:inline font-medium">{uiText.chatBtn}</span>
  ...
  return (
    <div className="fixed bottom-24 right-4 md:bottom-6 md:right-6 w-[90vw] max-w-[350px] bg-white rounded-2xl shadow-2xl flex flex-col border border-gray-200 z-50 overflow-hidden font-sans h-[500px] max-h-[80vh]">
      {/* Header */}
      ...
      <div className="p-3 bg-white border-t border-gray-100 flex flex-col gap-2">
        ...
        {pendingImage && (
          <div className="flex items-center gap-2 bg-indigo-50 p-2 rounded-lg border border-indigo-100 animate-slide-up">
            ...
            <div className="flex-1">
              <p className="text-xs font-medium text-indigo-900">{uiText.imageAttached}</p>
              <p className="text-[10px] text-indigo-600">{uiText.analyzing}</p>
            </div>
        ...
        <div className="flex gap-2">
          ...
          <input
            ...
            placeholder={pendingImage ? (language === 'hi' ? 'फोटो के बारे में पूछें...' : "Ask about this image...") : uiText.placeholder}
