...
export const analyzeUserImage = async (base64Image: string, language: Language = 'en'): Promise<AnalysisResult> => {
  ...
  const prompt = `
    ...
    First, check if the image actually contains a person (face/body). If it is a blank image, an irrelevant object, a landscape, or completely unrelated to a fashion analysis of a person, set "isRelevant" to false and provide a "relevanceMessage" explaining that the photo is invalid or inappropriate for this section. If it is relevant, set "isRelevant" to true and "relevanceMessage" to empty string.
    
    If the image is relevant, identify:
    1. Face Shape
...
export const matchOutfit = async (topImage: string, bottomImage: string, language: Language = 'en'): Promise<OutfitMatchResult> => {
  ...
  const prompt = `
    ...
    First, check if both images actually contain clothing items. If any of the images is blank, an irrelevant object, a landscape, or completely unrelated to clothing or fashion matching, set "isRelevant" to false and provide a "relevanceMessage" explaining that the photo is invalid or inappropriate for this section. If they are relevant, set "isRelevant" to true and "relevanceMessage" to empty string.
    
    If the images are relevant, check for color harmony, pattern clashing, and style consistency.
...
export const getSkincareAdvice = async (skinType: string, language: Language = 'en'): Promise<SkincareRoutine> => {
  ...
  try {
    ...
    console.error("Skincare Error:", e);
    throw e;
  }
};
...
export const chatWithStylist = async (history: any[], message: string, image?: string, language: Language = 'en') => {
  ...
  try {
    ...
    if (message.trim()) {
      ...
      msgParts.push({ text: "Analyze this image and provide styling advice based on our conversation." });
    }

    if (msgParts.length === 0) return language === 'hi' ? "कृपया कुछ लिखें।" : "Please say something.";
